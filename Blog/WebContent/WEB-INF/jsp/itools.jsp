...
<head>
	...
	<link rel="stylesheet" href="assets/css/itools.css">
...
<body>
	<header id="navigation-wrapper">
		<div class="fixed">
			...
			<nav class="topic">
				<ul>
					<c:forEach items="${itoolsTitle }" var="itoolsTitle">
						<li tab="${itoolsTitle.itoolsId }">
							<a href="" onclick="start(this)" name="${itoolsTitle.itoolsTitle }">${itoolsTitle.itoolsTitle }</a>
						</li>
					</c:forEach>
				</ul>
			</nav>
	...
	<div id="home" class="view">
		<div class="container">
			<div class="main">
				<c:forEach items="${itoolsTitle }" var="itoolsTitle">
					<div class="series-carousel-card" id="${itoolsTitle.itoolsId }">
					</div>
				</c:forEach>
			</div>
		</div>
		...
		function start(obj) {
			...
			$.ajax({
				...
				success: function (result) {
					debugger;
					var html = '<header class="series-carousel-header">\
				        <div>\
						    <h1 style="font-size: 24px; display: inline; margin-right: 20px;font-weight: bold;">在线工具</h1>\
							    <div style="float: right;"><span class="icon i-prev"></span> <span class="icon i-next"></span></div>\
					    </div>\
				  	    <div></div>\
				    </header><section id="wrapper" class="content"><div class="series-card-wrapper" style="transition-timing-function: cubic-bezier(0.1, 0.57, 0.1, 1); transition-duration: 0ms; transform: translate(0px, 0px) translateZ(0px);">';
					for (var i = 0; i < result.length; i++) {
						html += '<div class="series-card-carousel">\
								<header class="card-content-header">\
									<h1 class="title">'+ result[i].itoolsName + '</h1> <img class="thumbnail" src="' + result[i].itoolsCover + '" lazy="loaded">\
									<div class="info">\
										<div style="text-align: left;"><img class="avatar" src="'+ result[i].itoolsAuthorPic + '" lazy="loaded">\
											<h3>'+ result[i].itoolsAuthor + '</h3>\
										</div>\
										<span class="status">\
											<i class="icon i-clock"></i>\
												'+ result[i].itoolsIntro + '\
										</span>\
									</div>\
								</header>\
								<div style="font-size: 14px; color: rgb(74, 74, 74); text-align: center; margin: 23px 0px 18px;">\
									类别：<span style="color: rgb(255, 67, 77);">'+ result[i].itoolsType + '</span>\
								</div>\
						        <button class="sub-btn">试一试</button>\
					        </div>';
					}
					html += '</div></section>';
					$(".series-carousel-card").html(html);
				},

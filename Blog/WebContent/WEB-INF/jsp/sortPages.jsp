				...
				<header>
					<div class="inner">
						<div class="recommend-topic">
							<%-- 	<c:forEach items="${getTopic3 }" var="getTopic3">
							<a href="/topic/158" class="topic-card large"><img class="banner" src="https://cdn.sspai.com/topic/5e7dbaf5-2b4f-d99b-acdc-bc0deb19215f.jpg?imageMogr2/quality/95/thumbnail/!828x445r/gravity/Center/crop/828x445" lazy="loaded">
								<div class="mask">
									<h4>${getTopic3.topic }</h4>
									<footer><em>${getTopic3.num } 篇文章</em> <button class="btn">收藏</button> </footer>
								</div>
							</a>
					    </c:forEach> --%>
						</div>
		...
		function start(obj) {
			...
			$.ajax({
				...
				success: function (result) {
					...
					for (var i = 0; i < result.length; i++) {
						html += '<a href="/topic/158" class="topic-card large"><img class="banner" src="https://cdn.sspai.com/topic/5e7dbaf5-2b4f-d99b-acdc-bc0deb19215f.jpg?imageMogr2/quality/95/thumbnail/!828x445r/gravity/Center/crop/828x445" lazy="loaded"> \
							  <div class="mask"> \
							  <h4>'+ result[i].topic + '</h4> \
							  <footer><em>'+ result[i].num + '篇文章</em> <button class="btn">收藏</button></footer> \
							  </div></a>';
					}
					$(".recommend-topic").html(html);
				},
			...
			$.ajax({
				...
				success: function (result) {
					...
					for (var i = 0; i < result.length; i++) {
						html += '<a href="javascript:;" onclick="afunction(this)" data-id="' + result[i].sortPagesIds + '-' + id + '" data-tar="' + id + '" rel="' + result[i].sortPagesIds + '">' + result[i].sortPagesTitles + '</a>';
					}
					div.append(html);
				...
				success: function (result) {
					...
					for (var i = 0; i < result.length; i++) {
	        		 ...
	        		 <img class="banner" src="https://cdn.sspai.com/topic/0e4264c5-1ccc-dbf7-5326-cf1478a6c732.jpg?imageMogr2/quality/95/thumbnail/!828x445r/gravity/Center/crop/828x445" lazy="loaded"> \
			   		 <div class="mask"> \
			         <h4>'+ result[i].topic + '</h4> \
			         <footer><em>'+ result[i].num + '篇文章</em> <button class="btn">Enter</button> </footer> \
			         </div></a>';
					}

...
　　                      第三，集中传递页面权重。
				...
				<ul>
					<c:forEach items="${SortPagesList }" var="SortPages">
						<li tab="${SortPages.sortPagesId }">
							<a href="" onclick="start(this)" name="${SortPages.sortPagesId }">${SortPages.sortPagesTitle }</a>
						</li>
					</c:forEach>
	...
	<div class="main">
		<c:forEach items="${SortPagesList }" var="SortPages">
			<div class="topic-plaza view" id="${SortPages.sortPagesId }">
				<header>
					<div class="inner">
				...
				<div>
					<div class="topic-list" id="allTopic_${SortPages.sortPagesId }" >					
	...
	function start(obj){
		...
		$.ajax({
              ...
              success: function(result){
              	$("#topicDiv"+m).remove();
                  var div = $('<div id="topicDiv'+id+'"></div>');
                  var html = '<a href="javascript:;" onclick="afunction(this)" data-id="1010-'+id+'" data-tar="'+id+'" class="active">全部</a>';
                  for(var i = 0;i < result.length;i++){
                     html += '<a href="javascript:;" onclick="afunction(this)" data-id="'+result[i].type+'-'+id+'" data-tar="'+id+'" rel="'+result[i].sortPagesIds+'">'+result[i].sortPagesTitles+'</a>';                    
                  }
                  div.append(html);
                  $('#'+id).find('nav.type').append(div);
                  m = id;
                  afunction($('#'+id).find('nav.type a:eq(0)').get(0));            
               },
 	...
 	function afunction(obj){		 				
 		var aid = $(obj).data('id');
 		var arrayid = aid.split('-');
 		var pid = $(obj).data('tar');		 
	    $.ajax({
	        url: "sortPagesChild.html",
	        data: {pid:arrayid[1],childid:arrayid[0]},
	        type: "post",
	        async: false,
	        dataType: 'json',//xml,html,script,jsonp,text
	        success: function(result){
	        ...
	        var html="";
	        for(var i=0;i<result.length;i++){
	        html += '<a href="" class="topic-card"> \
	        		 <img class="banner" src="https://cdn.sspai.com/topic/0e4264c5-1ccc-dbf7-5326-cf1478a6c732.jpg?imageMogr2/quality/95/thumbnail/!828x445r/gravity/Center/crop/828x445" lazy="loaded"> \
			   		 <div class="mask"> \
			         <h4>'+result[i].topic+'</h4> \
			         <footer><em>6 篇文章</em> <button class="btn">收藏</button> </footer> \
			         </div></a>';
			    }
	               $("#allTopic_"+pid).append(html);
	            },
	            error:function(a,b,c){
	            alert(c);
	            }         
